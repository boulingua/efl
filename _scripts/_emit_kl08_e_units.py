"""Batch-emit Track E Klasse 8 — all 12 Units.

Niveau E version of Klasse 8: shares cast (Jonas, Hawa, global
pen-pal class) and theme arc with G+M, with one extra grammar
nuance per Unit and slightly richer texts. Bildungsplan prefix
3.2 (Klassen 7/8/9).
"""
...
COURSE = REPO / "track_e_kl08" / "units"

# Reuse most data from kl08 G+M with light enrichment.
# Imports from sibling script keep this file compact.
import importlib.util
GM_PATH = REPO / "_scripts" / "_emit_kl08_gm_units.py"
spec = importlib.util.spec_from_file_location("kl08_gm", GM_PATH)
gm_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(gm_module)
UNITS_GM = gm_module.UNITS

# Make a copy and tweak fields for Niveau E.
UNITS = []
for u_gm in UNITS_GM:
    u = {k: v for k, v in u_gm.items()}
    UNITS.append(u)
...
**Niveau:** E. Klassenarbeit at Niveau E (45 BE).
:::
...
def emit() -> None:
    ...
    print(f"Wrote {len(UNITS) * 3} files for Track E Klasse 8.")
