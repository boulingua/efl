...
- Quarto setup removed (commit `8d75f0a`):
  - `_quarto.yml`, all 585 `.qmd` files, `_extensions/`,
    `_includes/`, `_test_card_files/`, `efl.Rproj`,
    `_scripts/vgwort.lua` (dead code), `_scripts/build_vgwort_manifest.py`
    (one-off), `.github/workflows/publish.yml.disabled`,
    `.quarto/`, `docs/`.
  - Kept: `_resources/*.yml` (curriculum data driving the placeholder
    generators), `_scripts/` Python tooling, top-level
    `HANDOVER.md` / `LEGAL.md` / `README.md` / `LICENSE` / `CITATION.cff`.
- URL parity check (vs the live Quarto site's `sitemap.xml`,
  `_scripts/verify_url_parity.py`):
  - 407 sitemap URLs → 227 resolved (1 direct + 226 aliases).
  - 180 missing — all `*_slides.html`, the documented Quarto-only
    reveal.js outputs (Phase 0 §8 decision 1).
  - 2 originally-missing developer-doc URLs (`/HANDOVER.html`,
    `/LEGAL.html`) given static refresh redirects to the GitHub
    source.
- Internal link audit (`_scripts/verify_internal_links.py`): **0 of
  15,446 internal href/src targets broken**, after three rounds of fixes:
  - `_scripts/absolutize_links.py` — reverse-mapped every
    `content/.../*.md` back to its original `.qmd` directory, resolved
    relative `.qmd`-derived link targets there, then forward-mapped
    through the Phase 2 slug rules. 603 links across 33 files. The
    Phase 2 migrator was also patched so a future re-run emits
    absolute URLs natively.
  - `layouts/shortcodes/downloads.html` now wraps slide and exam-PDF
    links in `if fileExists` so they only emit when the artefact is
    actually present. When a real slide pipeline lands at the
    canonical path, the links reappear automatically. The unit page
    still shows the article + worksheet (which IS regenerated by
    `make_placeholder_worksheets.py`) lines unconditionally.
  - Coder theme's hardcoded favicon / webmanifest / avatar
    references overridden in
    `layouts/_partials/head/custom-icons.html` and the unset
    `params.avatarURL`. Re-introduce real assets in a future commit
    by deleting the override.
- VG Wort end-to-end: still **399 / 399** verified
  (manifest → content/ → public/).
- Build status: 440 pages, 731 static files, 430 aliases, **zero
  errors, zero warnings (`hugo --printPathWarnings`).**
- CI workflow gains five post-build verification steps:
  - `verify_all_pixels.py` (manifest → content/, blocking).
  - `verify_rendered_pixels.py` (manifest → public/, blocking).
  - `verify_url_parity.py` (deployed sitemap → public/,
    non-blocking — slide-deck gap is documented and not a
    deploy-blocker).
  - `verify_internal_links.py` (zero internal broken links,
    blocking).
  - `lychee-action` (external links, non-blocking initially with
    allowlist 200/204/206/301/302/308/403).
- Visual smoke check: deferred. Local Hugo dev-server build is clean;
  visual inspection on the deployed PR build is recommended before
  merge.

**Migration complete on branch `migration/hugo-coder`. PR open;
